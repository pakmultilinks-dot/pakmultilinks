...
import { ChevronDown, LoaderCircle, MapPin, Package, Phone } from "lucide-react";
...
export function QuotesTable({ initialRows, connected }: { initialRows: AdminQuoteRow[]; connected: boolean }) {
  const [serverRows, setServerRows] = useState(initialRows); const [pending,setPending] = useState(""); const [message,setMessage] = useState("");
  const localJson = useLocalRecordJson("pmh.quotes.v1");
  const localResult = useMemo(() => { try { const local = JSON.parse(localJson) as LocalQuote[]; return { rows: local.map((quote) => ({ id:String(quote.id),quoteNumber:String(quote.quoteNumber||quote.id),customerName:String(quote.customerName||""),companyName:String(quote.companyName||""),email:String(quote.email||""),phone:String(quote.phone||""),city:String(quote.city||""),notes:String(quote.notes||""),adminNotes:"",status:String(quote.status||"New").toUpperCase(),createdAt:String(quote.createdAt||new Date().toISOString()),items:Array.isArray(quote.items)?quote.items.map((item)=>({name:String(item.product||item.productName||"Requirement"),quantity:Number(item.quantity||1),details:String(item.details||"")})):[] })), error: "" }; } catch { return { rows: [] as AdminQuoteRow[], error: "Browser-local quotation records could not be read." }; } }, [localJson]);
  const rows = connected ? serverRows : localResult.rows;
  async function update(row:AdminQuoteRow,status:string) { setPending(row.id);setMessage(""); if(!connected){try{const local=JSON.parse(localStorage.getItem("pmh.quotes.v1")||"[]") as LocalQuote[];localStorage.setItem("pmh.quotes.v1",JSON.stringify(local.map((quote)=>quote.id===row.id?{...quote,status:title(status)}:quote)));window.dispatchEvent(new Event("pmh-local-records"));setMessage("Browser-local development status updated. This is not a server-backed operation.");}catch{setMessage("Local status could not be updated.");}setPending("");return;} try{const response=await fetch(`/api/quotes/${encodeURIComponent(row.id)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status,adminNotes:row.adminNotes})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Status could not be updated.");setServerRows((current)=>current.map((item)=>item.id===row.id?{...item,status:result.quote.status}:item));}catch(error){setMessage(error instanceof Error?error.message:"Status could not be updated.");}setPending(""); }
  if(!rows.length)return <EmptyState title="No quotation requests found" body={connected?"New corporate and bulk enquiries will appear here.":"No browser-local development quotation requests exist in this browser."}/>;
  return <>{(message || localResult.error)&&<p role="status" className="mb-4 rounded-xl bg-sky-50 p-3 text-sm text-sky-900">{message || localResult.error}</p>}<div className={`${panelClass} divide-y divide-slate-100 overflow-hidden`}>{rows.map((row)=><details key={row.id} className="group"><summary className="grid cursor-pointer list-none gap-3 p-5 hover:bg-slate-50 sm:grid-cols-[1fr_1fr_1fr_.7fr_auto] sm:items-center"><div><p className="font-bold text-emerald-900">{row.quoteNumber}</p><p className="text-xs text-slate-500">{new Date(row.createdAt).toLocaleString("en-PK")}</p></div><div><p className="font-semibold">{row.companyName}</p><p className="text-xs text-slate-500">{row.customerName}</p></div><p className="truncate text-sm text-slate-600">{row.city} · {row.phone}</p><span className="w-fit rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{title(row.status)}</span><ChevronDown className="size-4 text-slate-400 transition group-open:rotate-180"/></summary><div className="border-t border-slate-100 bg-slate-50/60 p-5"><div className="grid gap-6 lg:grid-cols-3"><div><h3 className="mb-2 text-xs font-bold uppercase text-slate-500">Requirements</h3><ul className="space-y-2 text-sm">{row.items.map((item,index)=><li key={index} className="flex justify-between gap-3"><span>{item.name}</span><strong>× {item.quantity}</strong></li>)}</ul></div><div className="text-sm"><h3 className="mb-2 text-xs font-bold uppercase text-slate-500">Contact & notes</h3><p>{row.email}</p><p>{row.phone}</p>{row.notes&&<p className="mt-3 leading-6 text-slate-600">{row.notes}</p>}</div><label className="text-sm font-semibold">Quotation status<div className="relative"><select value={row.status} disabled={pending===row.id} onChange={(event)=>update(row,event.target.value)} className="mt-1 h-11 w-full appearance-none rounded-xl border border-slate-200 bg-white px-3 pr-9 text-sm font-semibold outline-none focus:border-emerald-700">{quoteStatuses.map((status)=><option key={status}>{status}</option>)}</select>{pending===row.id&&<LoaderCircle className="absolute right-3 top-4 size-4 animate-spin"/>}</div></label></div></div></details>)}</div></>;
}
